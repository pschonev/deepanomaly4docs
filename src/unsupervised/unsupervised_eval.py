...
def eval_unsup(eval_run) -> None: 
    ...
    for test_data in eval_run.data:
        ...
        for data_params_ in test_data.cartesian_params():
            ...
            for model in eval_run.emb_model:

                # get document vectors from model
                docvecs = model.vectorize(df)

                for dim_reduction in eval_run.dim_red:
                    for dim_reduction_params_ in dim_reduction.cartesian_params():
                        ...
                        for outlier_predictor in eval_run.od_det:
                            for outlier_predictor_params in outlier_predictor.cartesian_params():
                                ...
                                add_scores(scores, [data_params_, model.__dict__, dim_reduction.__dict__, dim_reduction_params_, outlier_predictor_params, {"outlier_detector": outlier_predictor.outlier_detector}])
